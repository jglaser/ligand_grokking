...
class JaxOutOfCoreSVM:
    ...
    def fit(self, X_ligand, X_protein, train_pairs, y):
        ...
        # Pre-compute indices for the anti-deadlock heuristic
        positive_indices = jnp.where(y == 1)[0]
        negative_indices = jnp.where(y == -1)[0]

        # --- JAX-Compliant Training Loop Definition ---
        # To avoid capturing large arrays as constants, we define the entire
        # epoch logic inside a single function that we can JIT. This function
        # takes the large data arrays as explicit arguments.
        ...
        def run_epoch(carry, indices, X_ligand, X_protein, train_pairs, y, pos_indices, neg_indices, n_samples):
            ...
            def update_step(alphas, w, b, i, j):
                # This logic is nested to close over the symbolic arrays
                ...
                def optimization_logic():
                    ...
                    def update_and_report_change():
                        ...
                        cond1 = (0 < alpha_i_new) & (alpha_i_new < self.C)
                        cond2 = (0 < alpha_j_new_clipped) & (alpha_j_new_clipped < self.C)

                        new_b = jax.lax.cond(cond1, lambda: b1,
                                             lambda: jax.lax.cond(cond2, lambda: b2, lambda: (b1 + b2) / 2.0))
                        
                        new_w = w + (alpha_i_new - alpha_i_old) * y[i] * x_i + delta_alpha_j * y[j] * x_j
                        return new_alphas, new_w, new_b, True

                    return jax.lax.cond(jnp.abs(delta_alpha_j) > self.epsilon,
                                        update_and_report_change,
                                        lambda: (alphas, w, b, False))

                return jax.lax.cond((H - L > self.epsilon) & (eta < 0),
                                    optimization_logic,
                                    lambda: (alphas, w, b, False))

            def epoch_body(carry, i):
                alphas, w, b, key, count = carry
                lig_idx_i, prot_idx_i = train_pairs[i]
                x_i = jnp.concatenate([X_ligand[lig_idx_i], X_protein[prot_idx_i]])
                error_i = jnp.dot(w, x_i) + b - y[i]
                ...
                def perform_update():
                    ...
                    def pick_opposite():
                        return jax.lax.cond(y[i] == 1, 
                                            lambda: jax.random.choice(subkey, neg_indices), 
                                            lambda: jax.random.choice(subkey, pos_indices))
                    def pick_any():
                        rand_j = jax.random.choice(subkey, n_samples)
                        return jnp.where(rand_j == i, (rand_j + 1) % n_samples, rand_j)
                    
                    j = jax.lax.cond((pos_indices.shape[0] > 0) & (neg_indices.shape[0] > 0),
                                     pick_opposite, pick_any)
            ...
            final_carry, _ = jax.lax.scan(epoch_body, carry, indices)
            return final_carry
        ...
        for iter_num in range(self.max_iter):
            ...
            # Pass all large data arrays as arguments to the JIT'd function
            final_carry = run_epoch(initial_carry, shuffled_indices, X_ligand, X_protein, 
                                    train_pairs, y, positive_indices, negative_indices, n_samples)
    ...
    def decision_function(self, X_ligand, X_protein, test_pairs):
        """
        Computes the decision function for a given set of sample pairs.
        Uses jax.vmap for efficient, parallel prediction.
        """
        lig_vecs = X_ligand.at[test_pairs[:, 0]].get()
        prot_vecs = X_protein.at[test_pairs[:, 1]].get()
    ...
    def predict(self, X_ligand, X_protein, test_pairs):
        """Predicts the class labels for a given set of sample pairs."""
        return jnp.sign(self.decision_function(X_ligand, X_protein, test_pairs))
